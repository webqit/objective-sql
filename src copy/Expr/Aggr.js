 ...
 * @imports
 */
import { Call, ReferenceInterface } from '@web-native-js/jsen';
...
export default class Aggr extends _mixin(Call, AggrInterface) {
	...
	static parse(expr, parseCallback, params = {}) {
		...
		if (aggrMatch = expr.trim().match(new RegExp('^(' + aggrMatchRegex + ')', 'i'))) {
			...
			instance.args.meta.vars.forEach(_var => {
				if (_instanceof(_var, ReferenceInterface)) {
					_var.canBeLiteral = true;
				}
			});
			return instance;
		}
