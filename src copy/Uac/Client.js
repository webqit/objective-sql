...
export default class Client {
	...
	static select(USER, tableName, columns) {
		...
		if (!columns.length || columns[0] === '*') {
			columns = Object.keys(_Factory.schema[databaseName][tableName].fields);
		}
        // ---------------
        // OBJECT_QUERY
        // ---------------
	...
	static getRelatedAccounts(USER, organicRights, priorityAccounts = []) {
		var account_Factory = _Factory.schema[this.databaseName || 'default'].account;
		var ACCOUNT_QUERY = {
			table: account_Factory,
			alias: 'ACCOUNT',
			select: [],
			where: [],
			orderBy: [],
		...
		ACCOUNT_QUERY.AUTHOR_USER_RELATIONSHIP_QUERY = {
			...
			on: [
				ACCOUNT_QUERY.alias + '.' + account_Factory.primaryKey + ' = AUTHOR_USER_RELATIONSHIP.' + account_Factory.primaryKey,
		...
		if (priorityAccounts) {
			...
			ACCOUNT_QUERY.select.push('FIND_IN_SET(' + ACCOUNT_QUERY.alias + '.' + account_Factory.primaryKey + ', "' + priorityAccounts.join(',') + '") AS priority');
