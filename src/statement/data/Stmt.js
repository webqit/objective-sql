...
export default class Stmt {
	 ...
	 * @inheritdoc
	 */
	static async getParse(expr, withUac, stmtClauses, parseCallback, params, callback, literalValidateCallback = null) {
		...
		if (parse.matches.length) {
			...
			await Promise.all(parse.matches.map(async (clause, i) => {
				...
				var _exprParse = null;
				if (clauseType === 'JOIN_CLAUSE') {
					var _exprParse = await parseCallback(_expr, null, {withUac});
			...
			for (const alias in TABLES) {
				...
				if (params.withUac && !table.isDerivedQuery()/* Then UAC would already be applied */) {
					table.interpreted = await parseCallback('(' + UACClient.select(params, SCHEMAS[alias], null/* USER */, table.getAssociateReferences().map(ref => ref.name)/* columns */) + ') AS ' + (alias || tableName), [Table], {withUac: false});
				}
				// -----------
				var arrowReferences;
				if ((arrowReferences = table.getAssociateReferences().filter(ref => ref instanceof ArrowReferenceInterface)).length) {
					...
					var stmt = '(SELECT' + (withUac ? ' WITH UAC' : '') 
						...
						+ ' ' + _joins.join(' ') 
						+ ') AS ' + table.getAlias();
					(table.interpreted/* possibly by UAC evaluation */ || table).interpreted = await parseCallback(stmt, [Table], {withUac});
				}
			}
