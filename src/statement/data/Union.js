...
export default class Union extends UnionInterface {
	...
	static async parse(expr, parseCallback, params = {}) {
		...
		if ((parse = Lexer.lex(expr, [' UNION([ ]+(ALL|DISTINCT))? '], paramsRegex)) && parse.matches.length) {
			...
			return new this(
				await parseCallback(selects.shift().trim()),
				await Promise.all(selects.map(async (select, i) => {
					return {
						select: await parseCallback(select.trim()),
