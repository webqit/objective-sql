 ...
 * @imports
 */
import { _mixin, _isArray } from '@webqit/util/js/index.js';
import { _from as _arrFrom, _pushUnique } from '@webqit/util/arr/index.js';
...
export default class Select extends _mixin(Stmt, SelectInterface) {
	 ...
	 * @return array
	 */
	getFields(resolve = false) { return this.exprs.SELECT_LIST; }
	 ...
	 * @return Object|array
	 */
	getTable() { return this.exprs.TABLE_REFERENCES; }
	 ...
	 * @return Object
	 */
	getWhere() { return this.exprs.WHERE_CLAUSE; }
	 ...
	 * @return array
	 */
	getJoins() { return this.exprs.JOIN_CLAUSE; }
	 ...
	 * @return array
	 */
	getGroupBy() { return this.exprs.GROUP_BY_CLAUSE; }
	 ...
	 * @return array
	 */
	getWindows() { return this.exprs.WINDOWS_CLAUSE; }
	 ...
	 * @return array
	 */
	getOrderBy() { return this.exprs.ORDER_BY_CLAUSE; }
	 ...
	 * @return string
	 */
	getOffset() { return this.exprs.OFFSET_CLAUSE; }
	 ...
	 * @return string
	 */
	getLimit() { return this.exprs.LIMIT_CLAUSE; }
	...
	static async parse(expr, parseCallback, params = {}) {
		...
		if (expr.trim().substr(0, 6).toLowerCase() === 'select') {
			...
			var aliases = [];
			var stmtParse = await super.getParse(expr, withUac, this.clauses, parseCallback, params, async (clauseType, _expr) => {
				if (clauseType === 'SELECT_LIST') {
					return Promise.all(Lexer.split(_expr, [',']).map(async field => {
						var field = await parseCallback(field.trim(), [Field]);
						aliases.push(field.getAlias());
						return field;
					}));
				} else if (clauseType === 'WINDOWS_CLAUSE') {
					var windowsByName = {};
					for (const window of Lexer.split(_expr, [','])) {
						...
						windowsByName[windowSplit[0].trim()] = await parseCallback(windowSplit[1].trim(), [Window]);
					}
					return windowsByName;
				} else if (clauseType === 'GROUP_BY_CLAUSE') {
					return await parseCallback(_expr, [GroupBy]);
				} else if (clauseType === 'ORDER_BY_CLAUSE') {
					return await parseCallback(_expr, [OrderBy]);
				} else if (clauseType === 'LIMIT_CLAUSE') {
